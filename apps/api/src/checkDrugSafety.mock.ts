import type { CheckDrugSafety, RecallHit, SafetyReport } from '@farmacovigia/shared';
  ...
  const bulletin = anyHit
    ? `Se detectó al menos un fármaco con retiro activo de la FDA. ` +
      drugs
        ...
        .map(
          (d) =>
            `${d.input}: retiro Clase ${d.activeRecalls[0].classification} — ${d.activeRecalls[0].reason}`,
        )
        .join(' ') +
